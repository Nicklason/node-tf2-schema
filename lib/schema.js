...
class Schema {
    ...
    getQualityIdByName (name) {
        for (const type in this.raw.schema.qualityNames) {
            ...
            if (this.raw.schema.qualityNames[type] === name) {
                return this.raw.schema.qualities[name];
            }
        }
     ...
     */
    getSkinById (id) {
        for (const name in this.raw.schema.paintkits) {
            if (!Object.prototype.hasOwnProperty.call(this.raw.schema.paintkits, name)) {
                continue;
            }

            if (this.raw.schema.paintkits[name] === id) {
                return name;
            }
        }
    ...
    getSkinIdByName (name) {
        ...
        return this.raw.schema.paintkits[name];
    }
    ...
    getName (item, proper = true) {
        ...
        if (item.paintkit) {
            ...
            for (let i = 0; i < this.raw.schema.paintkits.length; i++) {
                const skin = this.raw.schema.paintkits[i];
